				...
				this.$api.imManageAPI
					...
					.then(res => {
						if(lodash.isEqual(res.code, "success")) {
							...
							this.info.data.data.mine.avatar = this.baseAddress + this.info.data.data.mine.avatar;
							
							for (var i = 0;i < this.info.data.data.friend.length;i++) {
								for (var j = 0;j < this.info.data.data.friend[i].list.length;j++) {
									this.info.data.data.friend[i].list[j].avatar = this.baseAddress + this.info.data.data.friend[i].list[j].avatar;
								}
							}
...
//					};
					...
					self.layim.on('sendMessage', function(data) {
						...
						// 保存消息
						self.$api.imManageAPI
							.sendMessage(paramMap)
//							.then(res => {
//								if(lodash.isEqual(res.code, "success")) {
//									console.log('发送成功');
//								}
